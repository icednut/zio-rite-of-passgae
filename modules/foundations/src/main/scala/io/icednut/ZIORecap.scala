...
object ZIORecap extends ZIOAppDefault {
  ...
  // fibers
  val delayedValue = ZIO.sleep(2.second) *> Random.nextIntBetween(0, 100)
  val aPair = for {
    a <- delayedValue
    b <- delayedValue
  ...
  object ConnectionPool {
    def live(nConnection: Int): ZLayer[Any, Nothing, ConnectionPool] = ZLayer.succeed(new ConnectionPool(nConnection))
  }
  ...
  val program = for {
    ...
    _ <- subscribe(User("Will", "will@rockthejvm.com"))
  } yield ()
